...
export default function AdminUploadPage() {
  ...
  function loadFile(file?: File) {
    if (!file) return;
    if (!file.name.endsWith(".mdx")) { setMessage("Hanya file .mdx yang diterima."); return; }
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent)]">GOXPOSISI Admin</p><h1 className="mt-3 text-3xl font-semibold text-[var(--foreground)]">Upload artikel</h1><p className="mt-2 text-[var(--muted)]">Unggah MDX, periksa preview, lalu publish ke Supabase.</p></div>
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="glass-panel rounded-2xl p-5">
          <label onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); loadFile(e.dataTransfer.files[0]); }} className={`flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-6 text-center transition ${dragging ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--border)]"}`}><span className="font-semibold text-[var(--foreground)]">Tarik file .mdx ke sini</span><span className="mt-2 text-sm text-[var(--muted)]">atau klik untuk memilih file</span><input type="file" accept=".mdx,text/markdown" className="sr-only" onChange={(e) => loadFile(e.target.files?.[0])} /></label>
          <textarea value={raw} onChange={(e) => setRaw(e.target.value)} placeholder="Atau tempel isi MDX di sini..." className="mt-5 min-h-72 w-full rounded-xl border border-[var(--border)] bg-[var(--surface-strong)] p-4 font-mono text-sm text-[var(--foreground)] outline-none focus:border-[var(--accent)]" />
